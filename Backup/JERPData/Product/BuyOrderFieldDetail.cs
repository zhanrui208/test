...
{
    /// <描述>
    /// 表[prd.BuyOrderFieldDetail]数据访问类
    ///</描述> 
    ///<作者> 
    /// 金优富
    ///</作者> 
    ...
    public class BuyOrderFieldDetail
    {
        ...
        public bool DeleteBuyOrderFieldDetailBatch(ref string ErrorMsg, object FieldTitleID)
        {
            ...
            try
            {
                ...
                SqlHelper.ExecuteNonQuery(DBTransaction, CommandType.StoredProcedure, "prd.DeleteBuyOrderFieldDetailBatch", arParams);
        ...
        public DataSet GetDataBuyOrderFieldDetailByFieldTitleID(long FieldTitleID)
        {
            ...
            {
                if (this.sqlConn.State == System.Data.ConnectionState.Closed) this.sqlConn.Open();
                ds = SqlHelper.ExecuteDataset(sqlConn, CommandType.StoredProcedure, "prd.GetDataBuyOrderFieldDetailByFieldTitleID", arParams);
            }
            catch//(SqlException ex)
            {
                // ex.Message --这里作调试用
        ...
        public bool InsertBuyOrderFieldDetail(ref string ErrorMsg, object FieldTitleID, object FieldID)
        {
            ...
            try
            {
                if (this.sqlConn.State == System.Data.ConnectionState.Closed) this.sqlConn.Open();
                DBTransaction = this.sqlConn.BeginTransaction();
                SqlHelper.ExecuteNonQuery(DBTransaction, CommandType.StoredProcedure, "prd.InsertBuyOrderFieldDetail", arParams);
                DBTransaction.Commit();
                flag = true;
            }
            catch (SqlException ex)
