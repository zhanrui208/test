    ...
    public class FileStatus
    {
        ...
        public bool DeleteFileStatus(ref string ErrorMsg, object FileID, object IsUseFlag, object DeptID)
        {
            ...
            try
            {
                ...
                SqlHelper.ExecuteNonQuery(DBTransaction, CommandType.StoredProcedure, "iso.DeleteFileStatus", arParams);
        ...
        public DataSet GetDataFileStatusByFileTypeID(int FileTypeID)
        {
            ...
            {
                if (this.sqlConn.State == System.Data.ConnectionState.Closed) this.sqlConn.Open();
                ds = SqlHelper.ExecuteDataset(sqlConn, CommandType.StoredProcedure, "iso.GetDataFileStatusByFileTypeID", arParams);
            }
            catch//(SqlException ex)
            {
                // ex.Message --这里作调试用
        ...
        public bool SaveFileStatus(ref string ErrorMsg, object FileID, object IsUseFlag, object DeptID, object Quantity)
        {
            ...
            try
            {
                if (this.sqlConn.State == System.Data.ConnectionState.Closed) this.sqlConn.Open();
                DBTransaction = this.sqlConn.BeginTransaction();
                SqlHelper.ExecuteNonQuery(DBTransaction, CommandType.StoredProcedure, "iso.SaveFileStatus", arParams);
                DBTransaction.Commit();
                flag = true;
            }
            catch (SqlException ex)
