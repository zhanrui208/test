...
{
    public partial class FrmExpenseAccountRpt : Form
    {
        public FrmExpenseAccountRpt()
        {
            InitializeComponent();
        ...
        private FrmExpenseAccountRecord frmRecord;
        ...
        public void LoadData()
        {
            ...
            this.dtblReport = this.accExpenseAccount.GetDataExpenseAccountExpenseRptPivotMonth(DateTime.Now.Year).Tables[0];
            string exp = string.Empty;
            if (this.dtblReport.Rows.Count > 0)
            {
                DataRow drowNew = this.dtblReport.NewRow();
                drowNew["ExpenseTypeID"] = -1;
                drowNew["ExpenseTypeName"] = "合计";
                for (int j = 1; j < 13; j++)
                {
        ...
        void dgrdv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                int ExpenseTypeID=(int)this.dtblReport .DefaultView [irow]["ExpenseTypeID"];
                int Month = int.Parse(this.dgrdv.Columns[icol].DataPropertyName);
                if (frmRecord == null)
                {
                    frmRecord = new FrmExpenseAccountRecord();
                    new FrmStyle(frmRecord).SetPopFrmStyle(this);                   
                }
                frmRecord.AccountRecord(ExpenseTypeID, DateTime .Now .Year ,
                    Month);
