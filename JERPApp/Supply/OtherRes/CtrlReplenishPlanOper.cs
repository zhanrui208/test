...
namespace JERPApp.Supply.OtherRes
{
    public partial class CtrlReplenishPlanOper : UserControl
    {
        ...
        void dgrdv_CellQuery(DataGridViewCellEventArgs e)
        {
            ...
            if (this.dgrdv.Columns[icol].Name == this.ColumnAssistantCode.Name)
            {
                ...
                if (this.frmGridPrd == null)
                {
                    ...
                    this.frmGridPrd.AddGridColumn("PrdName", "耗材名称", 100);
        ...
        void btnPrd_Click(object sender, EventArgs e)
        {
            if (frmAddPrd == null)
            {
                frmAddPrd = new JERPApp.Define.OtherRes.FrmProduct();
        ...
        {
            int PrdID = (int)drow["PrdID"];
            if (this.dtblPlans .Select("PrdID=" + PrdID.ToString(), "", DataViewRowState.CurrentRows).Length > 0) return;
