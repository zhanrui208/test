...
namespace JERPApp.Define.Manufacture
{
    ...
    /// 2014-11-23 16:21:19
    ///</时间> 
    public partial class CtrlWorkingSheetForFinishedPrd : UserControl
    {
        public CtrlWorkingSheetForFinishedPrd()
        {
            InitializeComponent();
            this.accDataWorkingSheets = new JERPData.Manufacture.WorkingSheets();
            this.cmbItem.SelectedIndexChanged += new EventHandler(cmbItem_SelectedIndexChanged);
            this.RefreshData();
        }
        private JERPData.Manufacture.WorkingSheets accDataWorkingSheets;
        private DataTable dtblWorkingSheets;
        private void RefreshData()
        {
            this.dtblWorkingSheets = this.accDataWorkingSheets.GetDataWorkingSheetsLastRecordForFinishedPrd (500).Tables[0];
        ...
        public long ManufPlanID
        {
            get
            {
                object objValue = this.GetFieldValue("ManufPlanID");
                if (objValue == DBNull.Value)
                {
                    return -1;
                }
        ...
        public bool PrdStoreFlag
        {
            get
            {
                object objValue = this.GetFieldValue("PrdStoreFlag");
                if (objValue == DBNull.Value)
                {
                    return false;
                }
        ...
        public bool MtrStoreFlag
        {
            get
            {
                object objValue = this.GetFieldValue("MtrStoreFlag");
                if (objValue == DBNull.Value)
                {
                    return false;
                }
